...
#endif
#undef CONFIG_VIVO_CHARGING_NEW_ARCH
...
void rtc_mark_bsptmode(void)
{
	unsigned long flags;

	rtc_xinfo("rtc_mark_bsptmode\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_SPAR0_BIT8, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}
...
void rtc_bbpu_power_down(void)
{
	...
	unsigned char exist;
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
	exist = get_platform_charger_detect();
#else
...
static void rtc_handler(void)
{
	...
	if (pwron_alarm) {
		unsigned long now_time, time, start_time;
				...
				do {
					...
					time =
					    mktime(tm.tm_year, tm.tm_mon,
						   ...
						   tm.tm_min, tm.tm_sec);
					pr_notice("now time : %04d/%02d/%02d %02d:%02d:%02d (%ld)\n",
						nowtm.tm_year, nowtm.tm_mon,
						nowtm.tm_mday, nowtm.tm_hour,
						nowtm.tm_min, nowtm.tm_sec,
						now_time);
					pr_notice("alarm time : %04d/%02d/%02d %02d:%02d:%02d (%ld)\n",
						tm.tm_year, tm.tm_mon,
						tm.tm_mday, tm.tm_hour,
						tm.tm_min, tm.tm_sec,
						time);
				} while ((time <= now_time) &&
				    (now_time < start_time + 3));
				spin_unlock_irqrestore(&rtc_lock, flags);
				/*
